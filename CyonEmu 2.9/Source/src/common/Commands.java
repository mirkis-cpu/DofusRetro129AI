...
public class Commands {
		...
		if(_compte.get_gmLvl() < 1)
		{
			...
			if(command.equalsIgnoreCase("HELP"))
			{
				...
				if (_compte.get_gmLvl() == 3)
				{
					...
					+			"<u>Give</u>"+" -  [Pseudo] [Nombre] [PA/PM/Chance/Inteligence/Force/Agilite] pour ajouté stats a le jouer\n";
						SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, mess);
						return;
				}
	...
	public void commandGmThree(String command, String[] infos, String msg) throws InterruptedException
	{
		...
		if(command.equalsIgnoreCase("UNBAN"))
		{
	        ...
	        SocketManager.SEND_Im1223_ALL("Le rate XP PvM a été modifié a " + cantidad);
	      }
		else
		...
		if(command.equalsIgnoreCase("DELNPC"))
		{
			...
			if (command.equalsIgnoreCase("CADEAU")) {
				int cadeau = 0;
				try {
					cadeau = Integer.parseInt(infos[1]);
				} catch (Exception e) {}
				...
				objetivo.get_compte().setCadeauID(cadeau);
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Don de " + cadeau + " à " + objetivo.get_name());
				...
				for (Personnage pj : World.getOnlinePersos()) {
					pj.get_compte().setCadeauID(cadeau);
				}
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Don de " + cadeau + " à tous les joueurs en ligne.");
			}
		 else
		...
		if(command.equalsIgnoreCase("ADDNPCITEM"))
		{
			...
			return;
		      ...
		      if (command.equalsIgnoreCase("GIVE"))
		      {
		        ...
		        String mess = "Vous vienne de donne "+nbr+" points de "+add+" a "+target.get_name();
				SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out,mess);
                SocketManager.GAME_SEND_STATS_PACKET(target);
		        SQLManager.SAVE_PERSONNAGE(target, false);
	    }else
	...
	public void commandGmFour(String command, String[] infos, String msg) throws InterruptedException
	{
		if(_compte.get_gmLvl() < 4)
		{
			...
			if(command.equalsIgnoreCase("POINTS"))
			{
				...
				if(infos.length == 3)//Si le nom du perso est spécifié
				{
					...
					perso.get_compte().setpoints(pointtotal);
					if(perso.isOnline())
						SocketManager.GAME_SEND_STATS_PACKET(perso);
				...
				if(command.equalsIgnoreCase("PLAYERIP"))
				{
					...
					if(command.equalsIgnoreCase("MAXPLAYER"))
					{
							...
							CyonEmu.CONFIG_PLAYER_LIMIT = num;
		    	...
		    	if(command.equalsIgnoreCase("SETADMIN"))
		{
			...
			if(gmLvl >= _compte.get_gmLvl()) { SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Impossible de definir un niveau GM >= au votre"); return; }
