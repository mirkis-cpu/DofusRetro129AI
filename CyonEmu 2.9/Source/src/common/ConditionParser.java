...
public class ConditionParser
{
	public static boolean validConditions(Personnage perso,String req)
	{
		...
		try
		{
				...
				jep.addVariable("CI", perso.getTotalStats().getEffect(Constants.STATS_ADD_INTE));
			 	...
			 	jep.addVariable("CM", perso.get_baseStats().getEffect(Constants.STATS_ADD_PM));
			 	//Stats de bases
			 	...
			 	boolean ok = false;
			 	if (perso.get_name().equals(Constants.RandomCondition))
			 	World.updateAccounts();
	...
	public static String havePO(String cond,Personnage perso)//On remplace les PO par leurs valeurs si possession de l'item
	{
		...
		if(CyonEmu.CONFIG_DEBUG) GameServer.addToLog("Entered Cond : "+cond);
		...
		if(CyonEmu.CONFIG_DEBUG) GameServer.addToLog("Returned Cond : "+copyCond);
		return copyCond;
	}
