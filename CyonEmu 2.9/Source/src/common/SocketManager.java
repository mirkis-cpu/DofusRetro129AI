...
public class SocketManager {
	...
	public static void REALM_SEND_GAME_SERVER_IP(PrintWriter out,int guid,boolean isHost)
	{
		...
		{
			String ip = CyonEmu.CONFIG_IP_LOOPBACK && isHost?CryptManager.CryptIP("127.0.0.1")+CryptManager.CryptPort(CyonEmu.CONFIG_GAME_PORT):CyonEmu.GAMESERVER_IP;
			packet += "XK"+ip+guid;
		}else
		{
			String ip = CyonEmu.CONFIG_IP_LOOPBACK && isHost?"127.0.0.1":CyonEmu.IP;
			packet += "YK"+ip+":"+CyonEmu.CONFIG_GAME_PORT+";"+guid;
		}
		send(out,packet);
		if(CyonEmu.CONFIG_DEBUG)
			RealmServer.addToSockLog("Conn: Send>>"+packet);
	...
	{
		StringBuilder packet = new StringBuilder();
		packet.append("ALK3160000000000|").append(persos.size());
	...
	public static void GAME_SEND_MAP_NPCS_GMS_PACKETS(PrintWriter out, Carte carte)
	{
		...
		if(packet.isEmpty() || packet.length() < 4)return;
		send(out,packet);
		if(CyonEmu.CONFIG_DEBUG)
			GameServer.addToSockLog("Game: Send>>"+packet);
	}
	...
	public static void SEND_Ew_PODS_MONTURE(Personnage perso, int pods) {
		String packet = (new StringBuilder("Ew")).append(pods).append(";5000")
				.toString();
		send(perso, packet);
		if (CyonEmu.CONFIG_DEBUG)
			GameServer.addToSockLog("Game: Send>>" + packet);
   ...
   public static void SEND_EL_LISTE_OBJ_DRAGO(PrintWriter out,
			Dragodinde drago) {
		String packet = (new StringBuilder("EL")).append(
				drago.getListeObjDrago()).toString();
		send(out, packet);
		if (CyonEmu.CONFIG_DEBUG)
   ...
   public static void GAME_SEND_FIGHT_CHANGE_OPTION_PACKET_TO_MAP(Carte map,char s,char option, int guid)
	...
	public static void SEND_SB_SPELL_BOOST(Personnage perso, String modif) {
	...
	public static void GAME_SEND_cMK_PACKET_TO_ALL(Personnage perso , String suffix,int guid,String name,String msg)
	{
		String packet = "cMK"+suffix+"|"+guid+"|"+name+"|"+msg;
        if (perso.get_lvl() < 6) {
        	SocketManager.GAME_SEND_MESSAGE(perso, "Ce canal n'est accessible qu'� partir du niveau <b>6</b>.", CyonEmu.CONFIG_MOTD_COLOR);
            GAME_SEND_BN(perso);
            return;
        }
		for(Personnage perso1 : World.getOnlinePersos())
			send(perso1,packet);
		if(CyonEmu.CONFIG_DEBUG)
	...
	public static void SEND_OAKO_PACKET(PrintWriter out, Objet obj) {
		String packet = "OAKO" + obj.parseItem();
		send(out, packet);
		if(CyonEmu.CONFIG_DEBUG)
			GameServer.addToSockLog("Game: Send>>"+packet);
	...
	public static void GAME_SEND_JO_PACKET(Personnage perso,ArrayList<StatsMetier> SMs)
	{
		for(StatsMetier sm : SMs)
		{
			String packet = "JO"+sm.getPosicion()+"|"+sm.getOptBinValue()+ "|" + sm.getSlotsPublico();
			send(perso,packet);
			if(CyonEmu.CONFIG_DEBUG)
				GameServer.addToSockLog("Game: Send>>"+packet);
		}
	...
	public static void GAME_SEND_FRIEND_ONLINE(Personnage friend, Personnage perso) 
	{
		String packet = "Im0143;"+friend.get_compte().get_pseudo()+
		" (<b><a href='asfunction:onHref,ShowPlayerPopupMenu,"+friend.get_name()+"'>"+friend.get_name()+"</a></b>)";
		send(perso, packet);
		if (CyonEmu.CONFIG_DEBUG)
		    GameServer.addToSockLog("Game: Send>>" + packet);
	}
    ...
    public static void REALM_SEND_AH_PACKET(PrintWriter out)
    {
      StringBuilder packet = new StringBuilder();
      packet.append("AH").append(CyonEmu.serverID).append(";").append(World.get_state()).append(";110;1").append('\000');
      send(out, packet.toString());
      if (CyonEmu.CONFIG_DEBUG)
    ...
    {
    	String packet = "AxK3160000000000";
      if (number > 0) {
        packet = packet + "|1," + number;
        packet += "|" + CyonEmu.serverID + "," + number;// Server ID
        }
      send(out, packet);
      if (CyonEmu.CONFIG_DEBUG)
        RealmServer.addToSockLog("Conn: Send>>" + packet);
	...
	public static void REALM_SEND_MESSAGE_MID(Personnage out, String str)
	{
		String packet = "M"+str;
		send(out,packet);
		if(CyonEmu.CONFIG_DEBUG)
	...
	public static void GAME_SEND_GJK_PACKET(Personnage out,int state,int cancelBtn,int duel,int spec,long time,int unknown)
	{
		StringBuilder packet = new StringBuilder();
		packet.append("GJK").append(state).append("|").append(cancelBtn).append("|").append(duel).append("|").append(spec).append("|").append(time).append("|").append(unknown);
		send(out,packet.toString());
	...
	public static void GAME_SEND_cMK_PACKET_INCARNAM_CHAT(Personnage perso, String suffix, int guid,
            String name, String msg) {
        String packet = "cMK" + suffix + "|" + guid + "|" + name + "|" + msg;
        if (perso.get_lvl() > 15 && perso.get_compte().get_gmLvl() < 2) {
            GAME_SEND_BN(perso);
            return;
        }
        for(Personnage perso1 : World.getOnlinePersos()) {
	...
	public static void GAME_SEND_Ag_PACKET(PrintWriter out, int idObjet, String codObjet) {
		String packet = "Ag1|" + idObjet
		+ "|Cadeau Dofus| Voil� un joli cadeau pour vous ! "
		+ "Un jeune aventurier comme vous sera sans servir de la meilleur fa�on ! "
	...
	public static void GAME_SEND_AGK_PACKET(PrintWriter out) {
		String packet = "AGK";
		send(out, packet);
		if (CyonEmu.CONFIG_DEBUG)
			System.out.println("Game: Send>> " + packet);
	...
	public static void SEND_Ej_LIVRE(Personnage pj,
			String str) {
	...
	public static void SEND_EW_METIER_PUBLIC(Personnage pj,
			String str) {
	...
	public static void SEND_GDF_PERSO(Personnage perso, int celda, int frame, int esInteractivo) {
		String packet = "GDF|" + celda + ";" + frame + ";" + esInteractivo;
		send(perso, packet);
		if (CyonEmu.CONFIG_DEBUG)
			System.out.println("ESTA TRITURADOR: PERSO>>  " + packet);
	}
	
	public static void SEND_EMK_MOVE_ITEM(PrintWriter out, char tipoOG, String signo, String s1) {
	...
	public static void SEND_OR_DELETE_ITEM(PrintWriter out, int id) {
		String packet = "OR" + id;
		send(out, packet);
	}
	...
	public static void GAME_SEND_MESSAGE_SERVER(Personnage out, String args)
	{
	...
	public static void GAME_SEND_Eq_PACKET(Personnage Personnage, long Prix)
	{ 
		StringBuilder Packet = new StringBuilder();
	...
	public static void SEND_MESSAGE_DECO(Personnage P, int MSG_ID, String args) {
    ...
    public static void PACKET_POPUP_ALL(String msg) {
		String packet = "BAIO" + msg;
		for(Personnage P : World.getOnlinePersos())
		{
		send(P, packet);
		}
   ...
   public static void PACKET_POPUP(Personnage perso, String str) {
   ...
   public static void SEND_gA_PERCEPTEUR(Personnage perso,
			String str) {
   ...
   public static void GAME_SEND_PERCO_INFOS_PACKET(Personnage perso, Percepteur perco, String car) 
    ...
    public static void GAME_SEND_QUEST_LIST(Personnage perso, String str) {
		String packet = "QL|442;1;5|220;0;1|185;0;3";
		send(perso, packet);
		if (CyonEmu.CONFIG_DEBUG)
			GameServer.addToSockLog("Game: Send>>"+str.toString()); 
	}
	public static void GAME_SEND_QUEST_SELECTION(Personnage perso, String str) {
		String packet = "QS220|253|2857,0;2858,0|||";
		send(perso, packet);
		if (CyonEmu.CONFIG_DEBUG)
			GameServer.addToSockLog("Game: Send>>"+str.toString()); 
	}
	public static void SEND_Wp_MENU_Prisme(Personnage perso) {
		String packet = "Wp" + perso.parsePrismesList();
		send(perso.get_compte().getGameThread().get_out(), packet);
	}
	public static void SEND_Ww_CLOSE_Prisme(Personnage out) {
		String packet = "Ww";
		send(out, packet);
	}
	public static void GAME_SEND_am_ALIGN_PACKET_TO_SUBAREA(Personnage perso, String str) {
		String packet = "am" + str;
		send(perso, packet);

	}
	public static void SEND_CB_BONUS_CONQUETE(Personnage pj, String str) {
		String packet = "CB" + str;
		send(pj, packet);

	}
	public static void SEND_Cb_BALANCE_CONQUETE(Personnage pj, String str) {
		String packet = "Cb" + str;
		send(pj, packet);

	}
	public static void SEND_GM_PRISME_TO_MAP(PrintWriter out, Carte Carte) {// envia informacion de todos mercantes en 1 Carte
		String packet = Carte.getPrismeGMPacket();
		if (packet.isEmpty())
			return;
		send(out, packet);
	}
	public static void GAME_SEND_PRISME_TO_MAP(Carte Carte, Prisme Prisme) {
		String packet = Prisme.getGMPrisme();
		for (Personnage z : Carte.getPersos())
			send(z, packet);

	}
	public static void SEND_CP_INFO_DEFENSEURS_PRISME(Personnage perso, String str) {
		String packet = "CP" + str;
		send(perso, packet);
	}
	
	public static void SEND_Cp_INFO_ATTAQUANT_PRISME(Personnage perso, String str) {
		String packet = "Cp" + str;
		send(perso, packet);
	}
	public static void SEND_CIV_CLOSE_INFO_CONQUETE(Personnage pj) {
		String packet = "CIV";
		send(pj, packet);

	}
	public static void SEND_CW_INFO_WORLD_CONQUETE(Personnage pj, String str) {
		String packet = "CW" + str;
		send(pj, packet);

	}
	public static void SEND_CIJ_INFO_JOIN_PRISME(Personnage pj, String str) {
		String packet = "CIJ" + str;
		send(pj, packet);

	}
	public static void GAME_SEND_aM_ALIGN_PACKET_TO_AREA(Personnage perso, String str) {
		String packet = "aM" + str;
		send(perso, packet);

	}
	public static void SEND_GA_ACTION_TO_CARTE(Carte Carte, String gameActionID, int actionID, String s1, String s2) {
		String packet = "GA" + gameActionID + ";" + actionID + ";" + s1;
		if (!s2.equals(""))
			packet += ";" + s2;
		for (Personnage z : Carte.getPersos())
	...
	public static void SEND_CS_SURVIVRE_MESSAGE_PRISME(Personnage perso, String str) {
		String packet = "CS" + str;
		send(perso, packet);

	}
	
	public static void SEND_CD_MORT_MESSAGE_PRISME(Personnage perso, String str) {
		String packet = "CD" + str;
		send(perso, packet);
	
	}
	public static void SEND_CA_ATTAQUE_MESSAGE_PRISME(Personnage perso, String str) {
		String packet = "CA" + str;
		send(perso, packet);

	}

public static void GAME_SEND_MESSAGE_TO_ALL_VIP(String msg, String color) {
    String packet = "cs<font color='#" + color + "'>" + msg + "</font>";
    for (Personnage P : World.getOnlinePersos()) {
        if (P.get_compte().get_vip() > 0) {
            send(P, packet);
        }
    }
    if (CyonEmu.CONFIG_DEBUG) {
