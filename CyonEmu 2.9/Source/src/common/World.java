...
public class World {
	...
	private static Map<Integer, Prisme> Prismes = new TreeMap<Integer, Prisme>();
	...
	public static void createWorld()
	{
		...
		System.out.println(MobTemplates.size()+" monstre ont ete chargees.");
		...
		System.out.println(NPCTemplates.size()+" NPC ont ete chargees.");
		...
		System.out.println(nbr+" fin actions ont ete charges.");
	    int numero = SQLManager.LOAD_PRISMES();
		...
		System.out.println(Animations.size() + " animation ont ete chargees.");
		SQLManager.LOGGED_ZERO();
		SQLManager.LOAD_ITEMS_FULL();
		SQLManager.LOAD_COMPTES();
		System.out.println(Comptes.size()+" comptes ont ete chargees.");
		SQLManager.LOAD_PERSOS();
		System.out.println(Persos.size()+" personnages ont ete chargees.");
		SQLManager.LOAD_GUILDS();
		System.out.println(Guildes.size()+" guildes ont ete chargees.");
		SQLManager.LOAD_MOUNTS();
		System.out.println(Dragodindes.size()+" montures ont ete chargees.");
		SQLManager.LOAD_GUILD_MEMBERS();
		nbr = SQLManager.LOAD_MOUNTPARKS();
		System.out.println(nbr+" enclos ont ete chargees.");
		nbr = SQLManager.LOAD_PERCEPTEURS();
		nbr = SQLManager.LOAD_HOUSES();
		System.out.println(nbr+" maisons ont ete chargeess.");
		nbr = SQLManager.LOAD_TRUNK();
		System.out.println(nbr+" coffres ont ete chargees.");
		nbr = SQLManager.LOAD_ZAAPS();
		System.out.println(nbr+" zaaps ont ete chargeess.");
		nbr = SQLManager.LOAD_ZAAPIS();
		System.out.println(nbr+" zaapis ont ete chargees.");
		SQLManager.LOAD_CHALLENGES();
		System.out.println(Challenges.toString().split(";").length+" challenges ont ete chargees.");
		nbr = SQLManager.LOAD_BANIP();
		System.out.println(nbr+" BAN_IP chargee.");
	...
	for (SubArea subarea : SubAreas.values()) {
		...
		if (first)
		 str += "|";
		 str += subarea.get_id() + ";" + subarea.get_alignement();
		 first = true;
		}
		return str;
	}
	...
	public static double getBalanceArea(Area area, int alignement) {
		int cant = 0;
		for (SubArea subarea : SubAreas.values()) {
			if (subarea.get_area() == area && subarea.get_alignement() == alignement)
				cant++;
		}
		if (cant == 0)
			return 0;
	...
	public static double getBalanceWorld(int alignement) {
		int cant = 0;
		for (SubArea subarea : SubAreas.values()) {
			if (subarea.get_alignement() == alignement)
				cant++;
		}
		if (cant == 0)
			return 0;
	...
	public static void updateAccounts() {
		for (Compte comptes : Comptes.values()) {
			SQLManager.UPDATE_ACCOUNT(comptes.get_GUID());
		}
	}
	...
	public static void saveAll(Personnage saver)
	{
	    ...
	    try
	    {
	      ...
	      {
	        if (c.getRealmThread() != null)
	          SocketManager.REALM_SEND_AH_PACKET(c.getRealmThread()._out);
	      }
	        System.out.println("Lancement de la sauvegarde du Monde...");
			SocketManager.GAME_SEND_Im_PACKET_TO_ALL("1164;");
			CyonEmu.isSaving = true;
			...
			for(Compte acc : Comptes.values())
			{
				Thread.sleep(25);//0.1 sec. pour 1 compte
				SQLManager.UPDATE_ACCOUNT_DATA(acc);//sauvegarde des comptes et de l'etable
			}
			
			Thread.sleep(500);
			
			System.out.println("Sauvegarde des Prismes");
			for (Prisme Prisme : Prismes.values()) {
				if (Cartes.get(Prisme.getCarte()).getSubArea().getPrismeID() != Prisme.getID())
			...
			for(Dragodinde t : Dragodindes.values())
			{
				if(t.get_id() > 0)
				{
					Thread.sleep(10);
					SQLManager.UPDATE_MOUNT_INFOS(t);
				}
			}
			...
			for(Personnage perso : Persos.values())
			{
				...
				Thread.sleep(25);//0.1 sec. pour 1 objets
				SQLManager.SAVE_PERSONNAGE(perso,true);//sauvegarde des persos et de leurs items
			}
			
			Thread.sleep(500);
			
			System.out.println("Sauvegarde des guildes...");
			for(Guild guilde : Guildes.values())
			{
				Thread.sleep(15);//0.1 sec. pour 1 guilde
				SQLManager.UPDATE_GUILD(guilde);
			}
			
			Thread.sleep(500);
			
			System.out.println("Sauvegarde des percepteurs...");
			...
			for(House house : Houses.values())
			{
				...
				{
					Thread.sleep(10);//0.1 sec. pour 1 maison
			...
			System.out.println("Sauvegarde des coffres...");
			...
			System.out.println("Sauvegarde des zones et sous-zones");
			...
			System.out.println("Sauvegarde des HDVs...");
			...
			if(saveTry < 10)
			{
				System.out.println("Relaod save");
				if(saver != null && _out != null)
					SocketManager.GAME_SEND_CONSOLE_MESSAGE_PACKET(_out, "Error reload saving");
				saveTry++;
				saveAll(saver);
			}
			else
			{
				...
				String mess = "Error " + saveTry + " reload...";
				if(saver != null && _out != null)
	...
	public synchronized static void addPrisme(Prisme Prisme) {
		Prismes.put(Prisme.getID(), Prisme);
	}
	
	public static Prisme getPrisme(int id) {
	...
	public static Collection<Prisme> AllPrisme() {
		if (Prismes.size() > 0)
	...
	public static String PrismesGeoposition(int alignement) {
		...
		for (Area area : Areas.values()) {
			...
			if (first)
				str += ";";
			str += area.get_id() + "," + area.getalignement() + ",1," + (area.getPrismeID() == 0 ? 0 : 1);
			first = true;
		}
		if (alignement == 1)
	...
	/*public static void showPrismes(Personnage perso) {
		for (SubArea subarea : SubAreas.values()) {
			if (subarea.get_alignement() == 0)
				continue;
			SocketManager.GAME_SEND_am_ALIGN_PACKET_TO_SUBAREA(perso, subarea.get_id() + "|" + subarea.get_alignement() + "|1");
		}
	}*/
	...
	public static int ipIsUsed2(String ip){
	...
	{
		for(int curID : CyonEmu.arenaMap)
		{
			if(curID == mapID)
				return true;
		}
	...
	public static void PriestRequest(Personnage perso, Carte carte, int IdPretre)
	{
		...
		if(Homme.getWife() != 0){
			SocketManager.GAME_SEND_MESSAGE_TO_MAP(carte, Homme.get_name()+" est déjà marier !", CyonEmu.CONFIG_MOTD_COLOR);
			return;
		}
		if(Femme.getWife() != 0){
			SocketManager.GAME_SEND_MESSAGE_TO_MAP(carte, Femme.get_name()+" est déjà marier !", CyonEmu.CONFIG_MOTD_COLOR);
			return;
		}
