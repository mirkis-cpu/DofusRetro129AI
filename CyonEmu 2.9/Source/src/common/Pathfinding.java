...
public class Pathfinding {
	...
	public static ArrayList<Fighter> getEnemyFighterArround(int cellID,Carte map,Fight fight)
	{
		...
		for(char dir : dirs)
		{
			Case c = map.getCase(GetCaseIDFromDirrection(cellID, dir, map, false));
			if(c == null) continue;
	...
	public static Fighter getEnemigoAlrededor(int celdaID, Carte mapa, Fight pelea) {
		char[] dirs = { 'b', 'd', 'f', 'h' };
		for (char dir : dirs) {
			Case sigCelda = mapa.getCase(GetCaseIDFromDirrection(celdaID, dir, mapa, false));
			if (sigCelda == null)
				continue;
			Fighter f = sigCelda.getFirstFighter();
			if (f != null) {
				if (f.getTeam() != pelea.getCurFighter().getTeam())
					return f;
			}
		}
		return null;
	...
	{ 
		...
		if(c == (char)('h'+1))
			c = 'a';
		Case cc = map.getCase(GetCaseIDFromDirrection(id,c,map,false));
		return cc != null ? cc.getFirstFighter() : null;
	}
	...
	private static Fighter getFighter2CellBefore(int CellID, char c,Carte map)
	{
		...
		Case cc2 = map.getCase(new2CellID);
		return cc2 != null ? cc2.getFirstFighter() : null;
	}
	...
	public static ArrayList<Case> getCellListFromAreaString(Carte map,int cellID,int castCellID, String zoneStr, int PONum, boolean isCC)
	{
		...
		switch(zoneStr.charAt(c))
		{
			...
			default:
				GameServer.addToLog("[FIXME]Type de port�e non reconnue: "+zoneStr.charAt(0));
			break;
		}
		return cases;
	}
	...
	public static int getNearestCellAround(Carte map,int startCell, int endCell, ArrayList<Case> forbidens)
	{
		...
		for(char d : dirs)
		{
			...
			if(dis < dist && map.getCase(c) != null && map.getCase(c).isWalkable(true) && map.getCase(c).getFighters().isEmpty() && !forbidens.contains(map.getCase(c)))
			{
				dist = dis;
				cellID = c;
			}
	...
	public static boolean checkLoS(Carte map, int cell1, int cell2,Fighter fighter, boolean isPeur)
	{
	...
	public static ArrayList<Fighter> getFightersAround(int cellID,Carte map,Fight fight)
	{
		...
		for(char dir : dirs)
		{
			Case ca = map.getCase(GetCaseIDFromDirrection(cellID, dir, map, false));
			if(ca == null) continue;
			Fighter f = ca.getFirstFighter();
			if(f != null)
				fighters.add(f);
		}
		return fighters;
