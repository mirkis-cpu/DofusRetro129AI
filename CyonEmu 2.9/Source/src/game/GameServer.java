...
public class GameServer implements Runnable{
	...
	private Timer _saveTimer;//Save
	private Timer _lastPacketTimer; //inactivité
	private long _startTime;
	private int _maxPlayer = 0;
	private Timer _ReboTimer; // Reboot automatique
	private Timer _loadActionTimer; // Boutique
			...
			if (CyonEmu.CONFIG_PUB == true) {
	            ...
	            _ReboTimer.schedule(new TimerTask() {
	                ...
	                public void run() {
	                    Thread t = new Thread(new SaveThread());
	                    t.start();
	                    SocketManager.PACKET_POPUP_ALL("Le reboot automatique va avoir lieu dans 30 minutes.");
	                    return;
	                }
	            ...
	            _ReboTimer.schedule(new TimerTask() {
	                ...
	                public void run() {
	                    Thread t = new Thread(new SaveThread());
	                    t.start();
	                    SocketManager.PACKET_POPUP_ALL("</b>Le reboot automatique va avoir lieu dans 5 minutes.<br />Sauvegardez votre personnage avec .save !");
	                    return;
	                }
