...
public class Personnage {
	...
	private int _sexe;
	private int _classe;
	public void set_classe(int _classe) {
	...
	private boolean _firstjoin = false;
	...
	private int _objetID = 0;
	private boolean _cass = false;
	//Fight end
	private boolean _hasEndFight = false;
	//Item classe
	private ArrayList<Integer> _itemClasse = new ArrayList<Integer>();
	private Map<Integer, Couple<Integer, Integer>> _itemClasseSpell = new TreeMap<Integer, Couple<Integer, Integer>>();
	private int _bendHechizo = 0;
	private int _bendEfecto = 0;
	private int _bendModif = 0;
	//Kolizeum
	private Coliseum Koli;
	// Taverne
	private long _timeInTaverne = 0;
	// GA
	...
	public static class Group
	{
		...
		public void leave(Personnage p)
		{
			...
			p.setGroup(null);
			_persos.remove(p);
			if(_chief == p && _persos.size() > 0) _chief = _persos.get(0);
			if(_persos.size() == 1)
			{
	...
	public static class Stats
	{
		...
		public static Stats cumulStat(Stats s1,Stats s2)
		{
			...
			for(int a = 0; a <= Constants.MAX_EFFECTS_ID; a++)
			{
				...
				if(a==Constants.STATS_ADD_PA && som>CyonEmu.pa)som=CyonEmu.pa;
				if(a==Constants.STATS_ADD_PM && som>CyonEmu.pm)som=CyonEmu.pm;
			...
			int mountXp,int mount,int honor,int deshonor,int alvl,String z, byte title, int wifeGuid)
	{
		...
		if(_curCarte == null && World.getCarte(CyonEmu.CONFIG_START_MAP) != null)
		{
			this._curCarte = World.getCarte(CyonEmu.CONFIG_START_MAP);
			this._curCell = _curCarte.getCase(CyonEmu.CONFIG_START_CELL);
		}else if (_curCarte == null && World.getCarte(CyonEmu.CONFIG_START_MAP) == null)
		{
	...
	public static Personnage CREATE_PERSONNAGE(String name, int sexe, int classe, int color1, int color2, int color3,Compte compte)
	{
		...
		Personnage perso = new Personnage(
				SQLManager.getNextPersonnageGuid(), name, sexe, classe,
				color1,
				color2,
				color3,
				CyonEmu.CONFIG_START_KAMAS,
				((CyonEmu.CONFIG_START_LEVEL-1)*1),
				((CyonEmu.CONFIG_START_LEVEL-1)*5),
				10000,
				CyonEmu.CONFIG_START_LEVEL,
				World.getPersoXpMin(CyonEmu.CONFIG_START_LEVEL),
				100,
				...
				(byte)0,
				"*#%!pi$:?",
				CyonEmu.CONFIG_START_MAP,
				CyonEmu.CONFIG_START_CELL,
				"",
				"",
				100,
				"",
	...
	public void setSpells(Map<Integer, SortStats> spells) {
		_sorts.clear();
		_sortsPlaces.clear();
		_sorts = spells;
		_sortsPlaces = Constants.getStartSortsPlaces(_classe);
	}
	...
	public boolean learnSpell(int spellID, int level, boolean save, boolean send)
	  {
	    ...
	    if (send)
	    {
	      SocketManager.GAME_SEND_SPELL_LIST(this);//TODO
	...
	public void OnJoinGame()
	{
		...
		if (_firstjoin)
		{
	      ...
	      if (CyonEmu.START_ITEMS == null)
		        return;
		      if (CyonEmu.START_ITEMS.contains(","))
		      {
		        for (String item : CyonEmu.START_ITEMS.split(","))
		        {
		          int qtd = Integer.valueOf(item.split(":")[1]).intValue();
		          int itemid = Integer.valueOf(item.split(":")[0]).intValue();
		          Objet.ObjTemplate obj = World.getObjTemplate(itemid);
		          if (obj == null)
		            continue;
		          Objet object = obj.createNewItem(qtd, false);
		          if (addObjet(object, true))
		            World.addObjet(object, true);
		        }
		      }
		      else
		      {
		        String item = CyonEmu.START_ITEMS;
		        int qtd = Integer.valueOf(item.split(":")[1]).intValue();
		        int itemid = Integer.valueOf(item.split(":")[0]).intValue();
		        Objet.ObjTemplate obj = World.getObjTemplate(itemid);
		        if (obj != null)
	...
	public String parseToGM()
	{
		...
		if(_fight == null)// Hors combat
		{
			...
			str.append((_onMount&&_mount!=null?_mount.get_color(parsecolortomount()):"")).append(";");			str.append(";");
		}
		return str.toString();
	}
	 ...
	 public String getGMStuffString()
	    {
	      ...
	      {
	       getObjetByPos(Constants.ITEM_POS_COIFFE).parseStatsString();
	       if(getObjetByPos(Constants.ITEM_POS_COIFFE).getObvijevanPos() != 10)
	        str.append(Integer.toHexString(getObjetByPos(Constants.ITEM_POS_COIFFE).getTemplate().getID()));
	    	   ...
	    	   int id_item;
				try {
					id_item = SQLManager.getAndDeleteLivingItem(getObjetByPos(Constants.ITEM_POS_COIFFE),false);
				} catch (SQLException e) {
					id_item = -1;
				}
				str.append(Integer.toHexString(id_item)+"~16~").append(
						getObjetByPos(Constants.ITEM_POS_COIFFE)
								.getObvijevanLook());       }
	      }
	      str.append(",");
	      if(getObjetByPos(Constants.ITEM_POS_CAPE) != null)
	      {
	       getObjetByPos(Constants.ITEM_POS_CAPE).parseStatsString();
	       if(getObjetByPos(Constants.ITEM_POS_CAPE).getObvijevanPos() != 11)
	        str.append(Integer.toHexString(getObjetByPos(Constants.ITEM_POS_CAPE).getTemplate().getID()));
	    	   ...
	    	   int id_item;
				try {
					id_item = SQLManager.getAndDeleteLivingItem(getObjetByPos(Constants.ITEM_POS_CAPE),false);
				} catch (SQLException e) {
					id_item = -1;
				}
				str.append(Integer.toHexString(id_item)+"~17~").append(getObjetByPos(Constants.ITEM_POS_CAPE).getObvijevanLook());       }
	      }
	      str.append(",");
    ...
    {
            if(CyonEmu.CONFIG_DEBUG)GameServer.addToLog("Perso "+_name+": tentative de boost stat "+stat + " " + countVal + " fois");
	...
	public void removeFromBank(int guid, int qua)
	{
	 ...
	 public void learnZobalSpells(Personnage _perso)
	 {
	...
	public void warpToSavePos()
	{
	  ...
	  public void set_statusDefaultWarp()
	  {
	    switch (_classe)
	    {
	    case 1:
	    ...
	    SocketManager.GAME_SEND_Im_PACKET(this, "06");
	  }
	...
	public void toogleOnMount()
	{
		...
		SocketManager.GAME_SEND_STATS_PACKET(this);
		int EnergyoLose = _mount.get_energie()- 60;
	...
	public void setInDD(boolean inDD) {
		_inDD = inDD;
	}
	...
	public void modifAlignement(int i)
	{
		...
		{
			SQLManager.UPDATE_GUILDMEMBER(getGuildMember());
			getGuildMember().setAlign((byte)i);
		}
	}
	...
	public String parsePrismesList() {
		...
		for (Prisme Prisme : World.AllPrisme()) {
			...
			if (World.getCarte(CarteID) == null)
				continue;
			if (World.getCarte(CarteID).getSubArea().get_area().get_superArea().get_id() != SubAreaID)
				continue;
	...
	public void usePrisme(String packet) {
		...
		if (CarteID == _curCarte.get_id())
			costo = 0;
		if (_kamas < costo) {
			SocketManager.GAME_SEND_MESSAGE(this, "Vous n'avez pas sufisamment de Kamas pour r�aliser cette action.",CyonEmu.CONFIG_MOTD_COLOR);
			return;
		}
		_kamas -= costo;
		SocketManager.GAME_SEND_STATS_PACKET(this);
	...
	public String parsePrisme() {
		String str = "";
		Prisme Prisme = World.getPrisme(_curCarte.getSubArea().getPrismeID());
		if (Prisme == null)
			str = "-3";
	...
	public void showRates() {
		SocketManager.PACKET_POPUP(this, "<b>Les rates du serveur sont :"
				+ "\nKamas : x" + CyonEmu.KAMAS 
				+ "\nDrop : x" + CyonEmu.CONFIG_DROP
				+ "\nExperience PvM : x" + CyonEmu.XP_PVM
				+ "\nRate PvP : x" + CyonEmu.HONOR 
				+ "\nExperience du metier : x"+ CyonEmu.XP_METIER
				+ "\nForge Magie : x" + CyonEmu.PORC_FM
				+ "\nComptes par IP : " + CyonEmu.PLAYER_IP + "</b>");
	}
	...
	public void itemagresion(Personnage perso) {
		long xp = Formulas.getTraqueXP(perso.get_lvl());
		perso.addXp(xp);
		long kamas = get_kamas() + (5000 * CyonEmu.KAMAS);
		perso.set_kamas(kamas);
		SocketManager.GAME_SEND_MESSAGE(perso, "Vous avez gagn� " + xp
				+ " points d'experience ansi que " + (5000 * CyonEmu.KAMAS) + " Kamas pour avoir honor� ton alignement.", "000000");
	...
	public void setMetierPublic(boolean publico) {
		_metierPublic = publico;
	}
	...
	public void setLivreArtisant(boolean viendo) {
		_livreArti = viendo;
	}
	...
	public void setObjetCass(int objetoID) {
		_objetID = objetoID;
	}
	
	public int getObjetCass() {
		return _objetID;
	}
	
	public void setCass(boolean romper) {
		_cass = romper;
	}
	...
	public Objet getitemGUID(int IDtemplate) {
		for(Entry<Integer, Objet> value : _items.entrySet())
		{
			Objet obj2 = World.getObjet(value.getKey());
	...
	public boolean addObjetWithOAKO(Objet objet, boolean Similer) {
		for (Entry<Integer, Objet> entry : _items.entrySet()) {
			Objet obj = entry.getValue();
			if (obj.getTemplate().getID() == objet.getTemplate().getID() && obj.getStats().isSameStats(objet.getStats())
					&& Similer && objet.getTemplate().getType() != 85 && obj.getPosition() == -1) {
		...
		_items.put(objet.getGuid(), objet);
		SocketManager.GAME_SEND_OAKO_PACKET(this, objet);
		return true;
	}
	
	public boolean addObjetSimiler(Objet objet, boolean hasSimiler, int oldID) {
		ObjTemplate objModelo = objet.getTemplate();
		...
		if (hasSimiler) {
			for (Entry<Integer, Objet> entry : _items.entrySet()) {
				Objet obj = entry.getValue();
				if (obj.getPosition() == -1 && obj.getGuid() != oldID && obj.getTemplate().getID() == objModelo.getID()
	...
	public Map<Integer, Couple<Integer, Integer>> getItemClasseSpell() {
	...
	public void addItemClasseSpell(int spell, int effect, int modif) {
		if (!_itemClasseSpell.containsKey(spell)) {
			_itemClasseSpell.put(spell, new Couple<Integer, Integer>(effect, modif));
		}
	}
	...
	public void refreshItemClasse() {
		for (int j = 2; j < 8; j++) {
			if (getObjetByPos(j) == null)
				continue;
			Objet obj = getObjetByPos(j);
			int template = obj.getTemplate().getID();
			int pano = obj.getTemplate().getPanopID();
	...
	public void setColiseum(Coliseum arena) {
		this.Koli = arena;
	}
	...
	{
	    _timeInTaverne = System.currentTimeMillis();
	}

	public void set_timeInTaverne(long num) {
	    _timeInTaverne = num;
	}
	  
	public long get_timeInTaverne()
	{
	    return _timeInTaverne;
	}
