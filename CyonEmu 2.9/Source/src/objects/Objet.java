...
public class Objet {
	...
	public static class ObjTemplate
	{
		...
		public ObjTemplate(int id, String strTemplate, String name, int type,int level, int pod, int prix, int panopID, String conditions,String armesInfos, int sold, int avgPrice, int points)
		{
			this.ID = id;
	...
	private ArrayList<String> SortStats = new ArrayList<String>();
	...
	public String parseStatsString()
	{
		...
		if ( (getTemplate().getPanopID() >= 81 && getTemplate().getPanopID() <= 92)
				|| (getTemplate().getPanopID() >= 201 && getTemplate().getPanopID() <= 212)) {
			for (String spell : SortStats) {
				...
				stats.append(sort[0]).append("#").append(sort[1]).append("#0#").append(sort[3]).append("#").append(World.getSort(spellid))/*.getNombre()*/;
				isFirst = false;
			}
		}
		...
		public void obvijevanNourir(Objet obj) {
			...
			for (Map.Entry<Integer, Integer> entry : Stats.getMap().entrySet())
			{
				...
				if (entry.getValue().intValue() > 500) // si le boost a une valeur supérieure à 500 (irréaliste)
					return;
				entry.setValue(Integer.valueOf(entry.getValue().intValue() + obj.getTemplate().getLevel() / 20));
			}
		}
	...
	public static int getPoidOfBaseItem(int i)//Donne le poid de l'item actuel
	{
		...
		ResultSet RS;
		try {
			RS = SQLManager.executeQuery("SELECT statsTemplate from `item_template` WHERE `id`='"+i+"';",CyonEmu.STATIC_DB_NAME);
			RS.next();
	...
	public static String getRunes(Objet Obj) {
		...
		for (String s : Splitted) {
			...
			if (numero <= 0)
				continue;
			if (random < Constants.getStatIDRune(statID))
				continue;
			...
			if (!isFirst)
				rune += ";";
			rune += Constants.statRune(statID, numero) + "," + quantity;
			isFirst = false;
		}
		return rune;
	}
