...
public class SpellEffect
	{
		...
		public void applyToFight(Fight fight,Fighter acaster, ArrayList<Fighter> cibles, boolean isCaC)
		{
			...
			if((fight.get_type() == Constants.FIGHT_TYPE_CHALLENGE) && (fight.get_challenges().size() > 0))
			{
		...
		private void applyEffect_787(ArrayList<Fighter> objetivos, Fight pelea) {
			...
			ArrayList<SpellEffect> EH = hechizo.getStatsByLevel(hechizoNivel)
					.getEffects();
			for (SpellEffect eh : EH) {
				for (Fighter objetivo : objetivos) {
					objetivo.addBuff(eh.effectID, eh.value, 1, 1, true, eh.spell,
							eh.args, caster);
				}
			}
			try {
		...
		private void applyEffect_185(Fight fight)
		{
			...
			if(fight.isOccuped(cellID))return;
				...
				if(!list.isEmpty()) 				 // d'un autre
					for(Fighter F : list){
						...
						if(origPos.get(F.getGUID()).getFirstFighter() == null) {
							F.get_fightCell().removeFighter(F);
		...
		private void applyEffect_8(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			target.get_fightCell().removeFighter(target);
			caster.get_fightCell().removeFighter(caster);
		...
		private void applyEffect_149(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			continue;
				if(spell == 686)
				{
	                if(target.getPersonnage() != null && target.getPersonnage().get_sexe() == 1 || target.getMob() != null && target.getMob().getTemplate().getID() == 547)
	                {
	                        id = 8011;
	                }
				}
		...
		private void applyEffect_176(ArrayList<Fighter> objetivos, Fight pelea) { 
			...
			for (Fighter objetivo : objetivos) {
				objetivo.addBuff(effectID, val, turns, 1, true, spell, args, caster);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(pelea, 7,
						Constants.STATS_ADD_PROS, caster.getGUID() + "",
						objetivo.getGUID() + "," + val + "," + turns);
			}
			try {
				Thread.sleep(200);
			} catch (InterruptedException e1) {
		...
		private void applyEffect_180(Fight fight)//invocation
		{
			...
			if(fight.isOccuped(cell))return;
			int id = fight.getNextLowerFighterGuid();
		...
		private void applyEffect_181(Fight fight)//invocation
		{
			...
			if(mobID == -1 || level == -1 || MG == null)return;
			int maxInvoc = 1 + caster.getTotalStats().getEffect(Constants.STATS_CREATURE);
			if(caster._nbInvoc >= maxInvoc)return;
			if(fight.isOccuped(cell))return;
            int id = fight.getNextLowerFighterGuid();
			MG.setInFightID(id);
		...
		private void applyEffect_152(Fight pelea, ArrayList<Fighter> objetivos) {
			...
			for (Fighter objetivo : objetivos) {
				objetivo.addBuff(effectID, val, turns, 1, true, spell, args, caster);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(pelea, 7, effectID,
						caster.getGUID() + "", objetivo.getGUID() + "," + val + ","
								+ turns);
			}
			try {
				Thread.sleep(200);
		...
		private void applyEffect_164(ArrayList<Fighter> objetivos, Fight pelea) { 
			...
			for (Fighter objetivo : objetivos) {
				objetivo.addBuff(effectID, val, turns, 1, false, spell, args,
						caster);
			}
			try {
				Thread.sleep(200);
		...
		private void applyEffect_220(ArrayList<Fighter> objetivos, Fight pelea) { 
			if (turns < 1)
				...
				for (Fighter objetivo : objetivos)
					objetivo.addBuff(effectID, 0, turns, 0, true, spell, args,
							caster);
			}
			try {
				Thread.sleep(200);
		...
		private void applyEffect_786(ArrayList<Fighter> objetivos, Fight pelea) {
			for (Fighter objetivo : objetivos)
				objetivo.addBuff(effectID, value, turns, 1, true, spell, args,
						caster);
			try {
				Thread.sleep(200);
			} catch (InterruptedException e1) {
		...
		private void applyEffect_671(ArrayList<Fighter> objetivos, Fight pelea) {
			if (turns <= 0) {
				Fighter objetivo = objetivos.get(0);
				if (objetivo.hasBuff(106) && objetivo.getBuffValue(106) >= spellLvl
						&& spell != 0) {
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(pelea, 7, 106,
							objetivo.getGUID() + "", objetivo.getGUID() + ",1");
					objetivo = caster;
				}
				if (objetivo.hasBuff(765)) {
					if (objetivo.getBuff(765) != null
							&& !objetivo.getBuff(765).getCaster().isDead()) {
						applyEffect_765B(pelea, objetivo);
						objetivo = objetivo.getBuff(765).getCaster();
					}
				}
				int resP = objetivo.getTotalStats().getEffect(
						Constants.STATS_ADD_RP_NEU);
				int resF = objetivo.getTotalStats().getEffect(
						Constants.STATS_ADD_R_NEU);
				if (objetivo.getPersonnage() != null) {
					resP += objetivo.getTotalStats().getEffect(
							Constants.STATS_ADD_RP_PVP_NEU);
					resF += objetivo.getTotalStats().getEffect(
							Constants.STATS_ADD_R_PVP_NEU);
				}
				int dano = Formulas.getRandomJet(args.split(";")[5]);
				int val = caster.getPDV() / 100 * dano;
				val -= resF;
				...
				if (val < 0)
					val = 0;
				val = applyOnHitBuffs(val, objetivo, caster, pelea);
				if (val > objetivo.getPDV())
					val = objetivo.getPDV();
				objetivo.removePDV(val);
				int cura = val;
				if (objetivo.hasBuff(786) && objetivo.getBuff(786) != null) {
					...
					SocketManager
							.GAME_SEND_GA_PACKET_TO_FIGHT(pelea, 7, 100,
									objetivo.getGUID() + "", caster.getGUID()
											+ ",+" + cura);
				}
				val = -(val);
		...
		private void applyEffect_143(ArrayList<Fighter> cibles, Fight fight) {
			if (turns <= 0) {
				...
				for (Fighter cible : cibles) {
					...
					if (healFinal < 1)
						healFinal = 0;
					cible.removePDV(-healFinal);
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 108, caster.getGUID() + "", cible.getGUID() + "," + healFinal);
					heal = dmg2;
				}
			} else {
				for (Fighter cible : cibles) {
					if (cible.isDead())
						continue;
					cible.addBuff(effectID, 0, turns, 0, true, spell, args, caster);
				}
			}
			try {
				Thread.sleep(200);
		...
		private void applyEffect_177(ArrayList<Fighter> objetivos, Fight pelea) { 
			...
			for (Fighter objetivo : objetivos) {
				objetivo.addBuff(effectID, val, turns, 1, true, spell, args, caster);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(pelea, 7,
						Constants.STATS_REM_PROS, caster.getGUID() + "",
						objetivo.getGUID() + "," + val + "," + turns);
			}
			try {
				Thread.sleep(200);
		...
		private void applyEffect_144(Fight pelea, ArrayList<Fighter> objetivos) { 
			...
			for (Fighter objetivo : objetivos) {
				objetivo.addBuff(145, val, turns, 1, true, spell, args, caster);
		...
		private void applyEffect_77(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			for(Fighter target : cibles)
			{
				...
				if(val < 1)continue;
				target.addBuff(Constants.STATS_REM_PM, val, 1,0, true, spell,args,caster);
		...
		private void applyEffect_765B(Fight fight,Fighter target)
		{
			...
			sacrified.get_fightCell().removeFighter(sacrified);
			target.get_fightCell().removeFighter(target);
