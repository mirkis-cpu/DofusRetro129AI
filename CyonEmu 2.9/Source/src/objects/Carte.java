...
public class Carte {
	...
	public static class Case
	{
		...
		public boolean canDoAction(int id)
		{
			switch(id)
			{
				...
				case 121:
				case 181:
					return _object.getID() == 7021;
				case 153 :
					return _object.getID() == 7352;
				default:
					return false;
			}
		}
		...
		public void startAction(Personnage perso, GameAction GA)
		{
			...
			switch(actionID)
			{
                ...
                break;
                case 153 :
					Trunk poub = Trunk.get_trunk_id_by_coord(0, 0);
					perso.setInTrunk(poub);
					Trunk.OpenTrunk(perso, "-", true);
				break;
                case 98://Vendre
				...
				case 170:// libro de artesanos
					...
					SocketManager.GAME_SEND_ECK_PACKET(perso, 14, "43;44;45;46;47;48;49;50;62;63;64");
				break;
				case 181:
					SocketManager.SEND_GDF_PERSO(perso, CcellID, 3, 1);
					SocketManager.GAME_SEND_ECK_PACKET(perso, 3, "1;181");
					perso.setCass(true);
				break;
				default:
	...
	public void startFightVersusMonstres(Personnage perso, MobGroup group)
	{
		int id = 1;
		if(!_fights.isEmpty())
			id = ((Integer)(_fights.keySet().toArray()[_fights.size()-1]))+1;
	...
	public void startFightVersusPercepteur(Personnage perso, Percepteur perco)
	{
		int id = 1;
		if(!_fights.isEmpty())
			id = ((Integer)(_fights.keySet().toArray()[_fights.size()-1]))+1;
	...
	public Fight startKolizeumFight(ArrayList<Personnage> team1, ArrayList<Personnage> team2) {
		int id = 1;
		if (!_fights.isEmpty())
			id = ((Integer)(_fights.keySet().toArray()[_fights.size()-1]))+1;
		Fight f = new Fight(id, this, team1, team2);
		_fights.put(id,f);
		return f;
	}
	
	public void startFightVSPrisme(Personnage perso, Prisme Prisme) {
		int id = 1;
		if (!_fights.isEmpty())
			id = ((Integer) (_fights.keySet().toArray()[_fights.size() - 1])) + 1;
	...
	public void addPlayer(Personnage perso)
	{
		...
		if(!perso.isDead())
		{
			if(perso.get_energy() >= 10000)return;
			if(Constants.isTaverne(this))
			{
				if(perso.get_timeInTaverne() == 0)
				{
					perso.set_timeInTaverne();
				}
			}else if(perso.get_timeInTaverne() != 0) {
				int gain = (int) ((System.currentTimeMillis() - perso.get_timeInTaverne()) / 1000);
				perso.set_energy(perso.get_energy() + gain);
				if(perso.get_energy() >= 10000) perso.set_energy(10000);
				SocketManager.GAME_SEND_Im_PACKET(perso, "092;" + gain);
				SocketManager.GAME_SEND_STATS_PACKET(perso);
				perso.set_timeInTaverne(0);
			}
		}
	}
