...
public class House
{
	...
	public static synchronized void HouseAchat(Personnage P)//Acheter une maison
	{
		House h = P.getInHouse();
		...
		if(h.get_owner_id() > 0)
		{
			...
			{
				SocketManager.GAME_SEND_MESSAGE(Seller.get_curPerso(), "Une maison vous appartenant à été vendue "+h.get_sale()+" kamas.", CyonEmu.CONFIG_MOTD_COLOR);
	...
	public static String parseHouseToGuild(Personnage P)
	{
		...
		for(Entry<Integer, House> house : World.getHouses().entrySet())
		{
			if(((House)house.getValue()).get_guild_id() == P.get_guild().get_id() && ((House)house.getValue()).get_guild_rights() > 0)
            	if (isFirst) {
					packet += house.getKey() + ";";
					if (World.getPersonnage(house.getValue().get_owner_id()) == null)
						packet += "Dueño privado;";
					else
						packet += World.getPersonnage(house.getValue().get_owner_id()).get_compte().get_pseudo() + ";";
					packet += World.getCarte((short) house.getValue().get_mapid()).getX() + ","
							+ World.getCarte((short) house.getValue().get_mapid()).getY() + ";";
					packet += "0;";
	...
	{	
		Boolean val = haveRight.get(rightValue);
		return val != null && val;
	}
