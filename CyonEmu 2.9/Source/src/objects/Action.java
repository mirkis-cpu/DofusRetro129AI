		...
		switch(ID)
		{
			...
			case 21://+Energie
				try
				{
					...
					/*int min = Integer.parseInt(this.args.split(",", 2)[0]);
					int max = Integer.parseInt(this.args.split(",", 2)[1]);
					if (max == 0) max = min;
					int val = Formulas.getRandomValue(min, max);
					int EnergyTotal = perso.get_energy()+val;
			...
			case 27://startFigthVersusMonstres args : monsterID,monsterLevel| ...
				...
				try
		        {
					...
					perso.get_curCarte().startFightVersusMonstres(perso, group);
		        }catch(Exception e){GameServer.addToLog(e.getMessage());};
			break;
			case 31:// change classe(zobal)
				try {
					...
					int level = perso.get_lvl();
					perso.setClase(classe);
					...
					perso.setSpells(Constants.getStartSorts(classe));
					Thread.sleep(150);
					perso.set_lvl(1);
					while (perso.get_lvl() < level) {
			   ...
			   case 32: // change sexe
				try {
					perso.changeSex();
					Thread.sleep(300);
					int morph = perso.get_classe() * 10 + perso.get_sexe();
					perso.set_gfxID(morph);
					SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(perso.get_curCarte(), perso.get_GUID());
					SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(perso.get_curCarte(), perso);
					SQLManager.CHANGE_SEXE(perso);
			   ...
			   case 33:// Stat max a un obj pos
					...
					Objet itemPos = perso.getObjetByPos(posItem);
					if (itemPos != null) {
						itemPos.clearStats();
				...
				if (perso.get_traque() == null) {
					traque t = new traque(0, null);
					perso.set_traque(t);
				}
				if (perso.get_traque().get_time() < System.currentTimeMillis() - 600000
						|| perso.get_traque().get_time() == 0) {
					Personnage tempP = null;
					ArrayList<Personnage> victimes = new ArrayList<Personnage>();
					for (Personnage victime : World.getOnlinePersos()) {
						if (victime == null || victime == perso)
							continue;
						if (victime.get_compte().get_curIP()
								.compareTo(perso.get_compte().get_curIP()) == 0)
							continue;
						if (victime.get_align() == perso.get_align()
								|| victime.get_align() == 0
								|| victime.get_align() == 3
								|| !victime.is_showWings())
							continue;
						if (((perso.get_lvl() + 20) >= victime.get_lvl())
								&& ((perso.get_lvl() - 20) <= victime.get_lvl()))
					...
					tempP = victimes.get(Formulas.getRandomValue(0,
							victimes.size() - 1));
           ...
           case 101://Arriver sur case de mariage
			...
			break;
			case 170:// Donné title
				try
			    {	
					byte title = (byte) Integer.parseInt(args); 
