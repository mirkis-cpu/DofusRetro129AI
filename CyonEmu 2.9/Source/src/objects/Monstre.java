...
public class Monstre
{
	...
	public static class MobGrade
	{
		...
		{
			float taux = (1.0F + (caster.get_lvl())/100.0F);
			int life = Math.round(PDVMAX * taux);
			PDV = life;
			PDVMAX = PDV;
			int force = stats.get(Constants.STATS_ADD_FORC)*(int)taux;
			int intel = stats.get(Constants.STATS_ADD_INTE)*(int)taux;
			int agili = stats.get(Constants.STATS_ADD_AGIL)*(int)taux;
			int sages = stats.get(Constants.STATS_ADD_SAGE)*(int)taux;
			int chanc = stats.get(Constants.STATS_ADD_CHAN)*(int)taux;
			stats.put(Constants.STATS_ADD_FORC, force);
