...
public class Metier
{
  ...
  public static class JobAction
  {
    ...
    public Case _cell;
    private InteractiveObject _IO;
    private StatsMetier _SM;
    ...
    public void endAction(Personnage P, InteractiveObject IO, GameAction GA,Case cell)
	{
		if(!_isCraft)
		{
			...
			if(System.currentTimeMillis() - _startTime < 500)return;
			IO.setState(3);
			IO.startTimer();
                  	...
                  	if(tID == protectors[i][1])
                   	{
                     	...
                     	int monsterLvl = Constants.getProtectorLvl(P.get_lvl());            
                      	P.get_curCarte().startFightVersusMonstres(P, new Monstre.MobGroup(P.get_curCarte()._nextObjectID, cell.getID(), monsterId+","+monsterLvl+","+monsterLvl));
                        break;
                 	}
                }
			}
    ...
    public void startAction(Personnage P, Carte.InteractiveObject IO, GameThread.GameAction GA, Carte.Case cell, StatsMetier SM)
    {
    ...
    private void doFmCraft()
    {
      ...
      {
        int chancesPA = Formulas.getRandomValue(1, 50);
        if (chancesPA != 10) 
        success = false;
      }
      ...
      if (!success)
      {
        ...
        SocketManager.GAME_SEND_REMOVE_ITEM_PACKET(_P, obj.getGuid());
        SocketManager.GAME_SEND_OAKO_PACKET(_P, obj);
        if (sign != null) SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(this._P, sign);
    ...
    public void resetCraft()
    {
    	...
    	_lastCraft.clear();
    	_IO.setState(1);
    	SocketManager.GAME_SEND_GDF_PACKET_TO_MAP(this._P.get_curCarte(), this._cell);
    	_cell = null;
    	_IO = null;
    	_SM.isOnAction = false;
    }
  }
  ...
  public static class StatsMetier
  {
    ...
    public JobAction get_JA(){
      return _curAction;
    }
    
    public InteractiveObject get_IO(){
      return _IO;
    }
