		...
		switch(_packetNum)
		{
			...
			case 3://HashPass
				...
				if(Compte.COMPTE_LOGIN(_accountName,_hashPass,_hashKey))
				{
					...
					if(CyonEmu.CONFIG_PLAYER_LIMIT != -1 && CyonEmu.CONFIG_PLAYER_LIMIT <= CyonEmu.gameServer.getPlayerNumber())
					{
						//Seulement si joueur
					...
					if(World.ipIsUsed2(ip) >= CyonEmu.PLAYER_IP)
					{
						SocketManager.REALM_SEND_TOO_MANY_PLAYER_ERROR(_out);
						try {
							_s.close();
			...
			default:
				...
				if(packet.substring(0,2).equals("Ax"))
				{
			        ...
			        if (!packet.equals("AX" + CyonEmu.serverID))
			          break;
			        CyonEmu.gameServer.addWaitingCompte(this._compte);
			        String ip = this._compte.get_curIP();
			        SocketManager.REALM_SEND_GAME_SERVER_IP(this._out, this._compte.get_GUID(), ip.equals("127.0.0.1"));
			      }
			    }
			  }
			}
